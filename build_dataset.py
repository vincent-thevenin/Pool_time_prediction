...
class PoolDataset(Dataset):
    def __init__(self, seq_len, heatup_seq_len=10, num_thread=None):
        self.turns = []
    ...
    def create_frames(self, turn_file, idx):
        ...
        for i,turn in enumerate(turn_list):
            for j,position in enumerate(turn):
                ...
                pos = np.dstack((x, y))
                rv = multivariate_normal([position[0]/(resolution[0]-1)*255, position[1]/(resolution[1]-1)*255], [[1.0, 0.0], [0.0, 1.0]]) #TODO Adapt variance
                frame = rv.pdf(pos)
    ...
    def __getitem__(self, idx):
        ...
        with ThreadPoolExecutor(num_thread) as executor:
            self.data = list(tqdm(executor.map(create_frames, self.turns), total=len(self.turns)))
        """
